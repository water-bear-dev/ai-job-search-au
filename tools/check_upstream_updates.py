...
3. Compares the 'framework_version' in your local files under
   skills/job-application-assistant/ with those in the upstream remote.
...
# Local path -> upstream path (AU fork uses skills/, upstream uses .claude/skills/)
UPSTREAM_PATH_MAP = {
    "skills/job-application-assistant/": ".claude/skills/job-application-assistant/",
}

def to_upstream_path(rel_path: str) -> str:
    for local_prefix, upstream_prefix in UPSTREAM_PATH_MAP.items():
        if rel_path.startswith(local_prefix):
            return upstream_prefix + rel_path[len(local_prefix):]
    return rel_path
...
def main() -> int:
    ...
    for rel_path in FRAMEWORK_FILES:
        ...
        if not local_path.exists():
            # AU fork: personalized skills/ is gitignored; fall back to tracked templates.
            alt = ROOT / "examples/profile" / rel_path
            if alt.exists():
                local_path = alt
            else:
        ...
        # Get upstream version (map local skills/ paths to upstream .claude/skills/)
        upstream_path = to_upstream_path(rel_path)
        rc, upstream_text, git_err = run_git(["show", f"{ref}:{upstream_path}"])
        if rc != 0:
    ...
    if updates_available:
        ...
        for up in updates_available:
            ...
            print(f"    Diff command: git diff {ref} -- {up.get('upstream_path', up['path'])}")
            print()
