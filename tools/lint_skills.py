...
Checks:
- Every SKILL.md (skills/*, .agents/skills/*) has YAML frontmatter that
...
- Every workflows/*.md starts with a `# /<name>` title
...
def main() -> int:
    skills = sorted(ROOT.glob("skills/*/SKILL.md")) + sorted(ROOT.glob(".agents/skills/*/SKILL.md"))
    ...
    if not commands and not workflows:
        errors.append("no command files found under .claude/commands/ or workflows/")
    ...
    if errors:
        ...
        for err in errors:
            print(f"  - {err}")
        return 1
    print(f"lint_skills: OK ({len(skills)} skills, {len(commands)} commands, {len(workflows)} workflows, settings.json)")
    return 0
