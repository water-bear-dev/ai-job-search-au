...
- Run `tools/parse_posting.py` on the user's URL or pasted text first (see `workflows/apply.md` Step 0)
- Fetch the job posting content (parser handles SEEK/LinkedIn; WebFetch for other URLs)
...
### Step 2: Tailor CV
- Read the most relevant existing CV variant from `applied_jobs/` or `cv/` as a starting point (any `applied_jobs/*/<FullName>_CV.tex`, legacy `cv/*/<FullName>_CV.tex`, or `cv/main_example.tex`)
- Follow the guidelines in `05-cv-templates.md`
- Create paths via `tools/application_paths.py --mkdir`, then write `applied_jobs/<application_folder>/<FullName>_CV.tex`
- Adjust: profile statement, skills section, experience bullet emphasis, section order

### Step 3: Write Cover Letter
- Follow the writing style rules in `03-writing-style.md` (critical: no em-dashes, no cliches)
- Follow the template structure in `06-cover-letter-templates.md`
- Create `applied_jobs/<application_folder>/<FullName>_CoverLetter.tex` (dated application subfolder; same folder as CV)
...
### Step 5: Update job tracker (when CV + cover letter were created)
- Run `tracker/upsert_application.py` with company, role, file paths, source URL, and fit rating (see `workflows/apply.md` Step 6a)
- Or rely on `/apply`, which does this automatically after PDF verification
