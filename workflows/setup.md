...
>
> I'll help you build your professional profile so the agent can evaluate job postings, tailor CVs, write cover letters, and prepare you for interviews.
>
...
Apply the confirmed changes by editing files in place (search/replace). Make targeted edits only. Do not rewrite entire files. State which changes were applied per file. If a file has no confirmed changes, state "No changes made to [filename]."
...
Then proceed to Step 3 to populate the non-skill files (`AGENTS.md`, `cv/main_example.tex`, `skills/job-scraper/search-queries.md`). Step 3 will detect that the seven skill files are already populated and skip those substeps.
...
- **Job portals:** "Discovery runs on SEEK via the built-in `tools/seek-search` CLI, plus optional startup boards (Wellfound, Y Combinator). Are there other sites you want to monitor? (e.g. a specific company's careers page)." SEEK is the default for the Australian market; LinkedIn/Indeed can be browsed manually and pasted into `/apply`.
